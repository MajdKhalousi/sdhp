...
>
> **Last verified against commit `63c1c31`** (Phase 157B documentation accuracy pass). The prior verification baseline was commit `13890c4` (Phase 138A) — 20 commits behind, during which Payroll shipped in full (148B-148D) and was previously misclassified here as "Not Started." Update this stamp whenever a future phase re-verifies this document's claims against the code.
...
The clinical and financial core is **mature and production-verified**: patients, appointments, doctor scheduling, queue/check-in, encounters, prescriptions, labs, radiology, medical files, clinical reports, allergies, medical timeline, billing/invoicing, and reporting are all classified **Production Ready** with confirmed automated test coverage and production-verification history (see [Product-Roadmap-State.md](architecture/Product-Roadmap-State.md)). The full reception → queue → doctor → encounter → billing → payment → reports workflow has been rehearsed end-to-end and passed. **HR (including Payroll) is the most recently and actively developed area** — employee profiles, attendance, leave, documents, and a full payroll-run lifecycle (DRAFT → APPROVED → PAID, bookkeeping only — no real payment execution) are all built and wired in; the only confidence gap is no confirmed automated test count for Payroll specifically. The platform/Super Admin layer (tenant onboarding, subscription billing, audit logs) is also Production Ready. **Phase 154B** closed a real security gap (DOCTOR/NURSE invoice access narrowed to their own clinical scope, deployed and production-verified) and fixed a billing/cashier report count inconsistency. **Local automated backups are verified healthy** in production (daily Postgres + MinIO, integrity-checked, 30-day retention confirmed); an offsite-replication script and runbook exist repo-only, deliberately deferred until real/pilot clinic data exists. The biggest gap relative to apparent completeness is that **Follow-up reminders and Notifications track state but never actually contact a patient** — no SMS/WhatsApp/email/push delivery exists anywhere in the system. Several named-but-unbuilt areas exist as empty placeholder folders (`notifications`, `ai-assistant`, `rooms`, `staff`, `staff-scheduling`) with zero implementation, and **Inventory/stock management does not exist at all**, despite being named in this project's original architecture brief as a core module. Current focus is **continued product development toward real daily clinic use, not a public demo.**
...
| **Mostly Complete** | Clinical Reports (functionally complete, no confirmed test-count) · HR — Employee Profiles/Attendance/Leave/Documents (missing biometric attendance only) · Payroll Calculation & Runs (full DRAFT→APPROVED→PAID lifecycle, bookkeeping only by design, no confirmed test-count) · Audit Logs (read API solid; writer coverage confirmed missing on doctor-schedules/branches/departments/clinic-settings as of Phase 155) |
...
- **Backup automation** (H1) — done: local automated backups are verified healthy in production (Phase 156A); offsite replication is designed and repo-ready but deliberately deferred until real/pilot clinic data exists.
- **Audit log coverage sweep** (E3) — Phase 155 confirmed the specific gap: `doctor-schedules`, `branches`, `departments`, and `clinic-settings` services write no audit entries at all. The pattern is established elsewhere (Employees/HR and Payroll both already write audit entries) — this is targeted injection into four services, not new design.
...
- **Same-org duplicate-phone detection** (B1) — Phase 155 finding: cheap, isolated, warn-and-confirm check at patient creation.
- **Reception next-available-slot UX** (B2) — Phase 155 finding: real daily friction observed firsthand during a reception-workflow rehearsal; the backend already has the conflict data needed.
...
| Audit log writer coverage confirmed missing on 4 config services | Medium — compliance-relevant for a medical-records system | `doctor-schedules`, `branches`, `departments`, `clinic-settings` write zero audit entries (confirmed, Phase 155). [Product-Roadmap-State.md §E3](architecture/Product-Roadmap-State.md) |
...
| Stale schema comment on `EmployeeDocument` | Low — documentation debt, not a functional bug | Comment says "no upload endpoint yet"; feature has shipped and was re-confirmed implemented twice (Phase 144C; re-verified Phase 157B). [Database Schema.md §4](architecture/Database%20Schema.md) |
...
| Same-org duplicate-phone detection not enforced on Patients | Medium — cheap to close | Confirmed, Phase 155. [Product-Roadmap-State.md §B1](architecture/Product-Roadmap-State.md) |
| Reception booking UX requires manual slot-guessing | Medium — cheap to close | Observed firsthand during a workflow rehearsal, Phase 155. [Product-Roadmap-State.md §B2](architecture/Product-Roadmap-State.md) |
| Offsite backup replication not yet installed in production | Low — deliberate, not an oversight | Script + runbook exist repo-only (Phase 156C); deferred by operator decision until real/pilot clinic data exists. [Deployment.md §6](architecture/Deployment.md) |
| Audit log table itself has no tamper-evidence | Low-Medium | Regular mutable table; no DB-level append-only enforcement. Flagged Phase 155, not yet addressed. [Product-Roadmap-State.md §E3](architecture/Product-Roadmap-State.md) |

...
> These are dev/staging demo accounts only, generated by seed data. They are not production credentials. The same seed (`apps/api/prisma/seed.ts`) populates both local development and the staging environment — see [`docs/04-Operations/Staging-Environment.md`](docs/04-Operations/Staging-Environment.md).
...
### Accountant
> Billing specialist. Manages cashier collections, invoices, and billing reports — no clinical or appointment access.
...
**Best for demoing:** cashier page (Today / Outstanding tabs), invoice collection, billing reports.

...
| 8 | Medical service requests | **Missing as a distinct concept** | No `MedicalServiceRequest` entity. A "service request" in Elaji today is just adding an `InvoiceItem` (referencing `Service`) to an invoice — there is no separate request date, requested-by, doctor, or **execution status** field independent of payment status. MedPro explicitly tracks payment status and "Executed?" as two separate fields (video 04) — Elaji has no equivalent for the execution side at all. |
...
| Medical service requests | Separate entity: request date, requested-by, doctor, payment status, **execution status** (independent of payment) | Only `InvoiceItem` exists — no request lifecycle, no execution tracking | Cannot track "service paid but not yet performed" or "performed but not yet billed" as distinct states | P1 |
...
| **MedicalServiceRequest** | Does not exist | **Candidate for addition** if Elaji wants execution tracking separate from billing (e.g. "service paid but not performed yet") — this is the clearest, most concrete gap found in this pass | Would relate to: Patient, Service, Doctor (nullable), Encounter/Appointment (nullable), InvoiceItem | Needs its own status lifecycle design before any schema work — see Sprint 2 below |
